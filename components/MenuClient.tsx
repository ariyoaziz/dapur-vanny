            ...
            <div className="flex justify-center mt-6 gap-3 flex-wrap">
                {[
                    ...
                    <button
                        ...
                        className={`px-4 py-1 rounded-full text-sm ${category === cat.value ? "bg-(--primary) text-white" : "border border-gray-200"}`}
                    >
                        {cat.label}
                    </button>
                ))}
            ...
            <div className="grid mt-14 gap-8 grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
                {paginatedMenu.length === 0 ? (
                    ...
                    paginatedMenu.map((item, index) => (
                        <ProductCard key={item.id} {...item} index={index} baseRoute={item.baseRoute} />
                    ))
                )}
            </div>
